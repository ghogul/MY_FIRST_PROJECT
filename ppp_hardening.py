...
def material_rotuine(poission_ratio, youngs_modulus,B_matrix,initial_displacement,i,j,global_plastic_strain,yield_stress,mu,lamda):
    ...
    print("n",trial_stress_deviatoric/np.linalg.norm(trial_stress_deviatoric))
    ...
    alpha = alpha_updated[i,j]
    beta = int(hardening_modulus*alpha)
    print("Beta", beta)
    ...
    if phi < 0:
        ...
        plastic_strain[i,j] = global_plastic_strain[i,j] + ((delta_lamda * 3/2) *(current_stress_deviatoric/current_stress_equivalent))
        # print("global plastic strain corrector",plastic_strain[i,j])
        alpha_updated[i,j] = alpha + (np.sqrt(2/3)*(delta_lamda))
...
def element_rotuine(radius,gauss_point):
    ...
    for i in range(nelm):
        ...
        for j in range(gauss_point):
            ...
            c_matrix,stress_matrix = material_rotuine(poission_ratio, youngs_modulus,B_matrix,initial_displacement,i,j,global_plastic_strain,yield_stress,mu,lamda)
